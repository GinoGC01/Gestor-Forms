...
export default function FormList() {
  const {Selected, hospitales, setHospitalesNav, setDataHospital, setGraficos} =
        ...
        {hospitales.map((hospital) => {
          ...
          return (
            <li className='li__Home' key={hospital.id}>
              <button
                className='btn-hospitales__Home'
                id={hospital.id}
                onClick={() => {
                  handleSelectedAndStateFormList(hospital)
                }}
              >
                <Link
                  className='text-btn__Home'
                  to={`/${hospital.id}`}
                >{`${hospital.nameVisible}`}</Link>
              </button>
              <button
                className='share__Home'
                onClick={() => shareFomr(HOSPITAL)}
              >
